...
import { statuses, treeHealthStatuses } from "../../../utils/appConstants";
import Pagination from "../../../components/common/pagination/Pagination";

function ExpertMonitorList() {
  const store = useSelector(selectExpertMonitor);
  const dispatch = useDispatch();
  const pageName = "Expert Monitors";
  const routeStart = "/expert-monitor";
  const breadCrumbs = [
    {
      name: "Dashboard",
      link: "/dashboard",
                  ...
                  <div className="card-body p-0">
                    <Table
                      data={store.list.map((listData, index) => ({
                        illnessName: listData.illness.name,
                        treeScanId: listData.tree.scanId,
                        files: listData.files ? listData.files.length : 0,
                        health: treeHealthStatuses[listData.healthStatus],
...
export default ExpertMonitorList;
